        ...
        setTimeout(() => {
            if (available >= quantity) {
                ...
                if (room.lastPlayer.diceCount === 0) {
                    io.to(room.lastPlayer.roomName).emit(
                        'playerOut',
                        room.lastPlayer.username,
                        playersOf(room).filter((p) => p.diceCount > 0).length,
                    )
                }
            }
...
const startTurn = (room) => {
    ...
    playersOf(room).forEach((playerOrSpectator) => {
        if (playerOrSpectator.diceCount > 0) {
            ...
            playerOrSpectator.emit(
                'turnStart',
                playerState,
                room.history,
                room.turn,
                playersOf(room).filter((player) => player.diceCount > 0)[room.turn].id === playerOrSpectator.id,
            )
        } else {
