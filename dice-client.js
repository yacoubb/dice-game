...
const client = (...args) => {
    ...
    const coloredCommands = client.commands
    Object.keys(coloredCommands).forEach((command) => {
        coloredCommands[command].highlight = 'blue'
    })

    registerCommands(dicegameCommands)
    registerCommands(coloredCommands)
    ...
    socket.on('bluff', (username, lastPlayerUsername, quantity, value, rolls) => {
        logger(
            `${chalk.bold.yellow(username)} called ${chalk.bold.yellow(lastPlayerUsername)}'s bluff of ${chalk.bold.green(
                quantity + ' ' + value,
            )}`,
        )
